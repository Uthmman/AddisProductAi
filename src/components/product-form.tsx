...
export default function ProductForm({ product }: ProductFormProps) {
  ...
  // Watch raw inputs to sync with preview state
  const watchPrice = form.watch('price_etb');
  const watchName = form.watch('raw_name');

  useEffect(() => {
    // Automatically update regular_price in aiContent if not currently being generated by AI
    if (generatingField !== 'regular_price' && generatingField !== 'all') {
      setAiContent(prev => ({ ...prev, regular_price: watchPrice }));
    }
  }, [watchPrice, generatingField]);

  useEffect(() => {
    // Automatically update name in aiContent if not currently being generated by AI
    if (generatingField !== 'name' && generatingField !== 'all') {
      setAiContent(prev => ({ ...prev, name: watchName }));
    }
  }, [watchName, generatingField]);
  ...
  const onSubmit = async (action: SaveAction) => {
    ...
    try {
        ...
        if (product) {
            ...
            // Compare price precisely as string
            if (finalData.regular_price !== product.regular_price) { 
                diff.regular_price = finalData.regular_price; 
                hasChanges = true; 
            }
             ...
             <Card>
                ...
                <CardContent className="space-y-4">
                    ...
                    <FormField control={form.control} name="raw_name" render={({ field }) => (
                        <FormItem><FormLabel>Product Name (Raw)</FormLabel><FormControl><Input placeholder="e.g., Traditional Bunk Bed" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                     <FormField control={form.control} name="material" render={({ field }) => (
                        <FormItem><FormLabel>Material</FormLabel><FormControl><Input placeholder="e.g., Solid Oak" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                    ...
                    <FormField control={form.control} name="amharic_name" render={({ field }) => (
                        <FormItem><FormLabel>Amharic Name</FormLabel><FormControl><Input placeholder="e.g., የባህል አልጋ" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                </CardContent>
            </Card>
